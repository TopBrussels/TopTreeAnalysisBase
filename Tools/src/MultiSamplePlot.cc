			   ...
			   float x1, float y1, float x2, float y2, float magnify)
{
  ...
  TH1F *hErrorPlus = 0, *hErrorMinus = 0, *hNominal = 0;
  if(addRatioErrorBand || addErrorBand){
  ...
  float integralData = 0, integralSM = 0;
  if ( dataPlotID != -1 ){
  ...
  vector<TH1F*> histosForHStack, histosForHStackAreaNorm, histosForOverlay, histosForOverlayAreaNorm;
  TH1F *totalSM = 0, *totalSMAreaNorm = 0;
  vector<Dataset*> datasetsForHStack, datasetsForOverlay;
  ...
  if(hData_){
      ...
      //padAreaNorm->SetGridy(0);
      padAreaNorm->SetGridx(0);
...
{ 
  ...
  if(hData_){
      ...
      else cmstext.DrawLatex(0.16,0.975,("  "+slumi.str()+" fb^{-1} at #sqrt{s} = "+ssqrts.str()+" TeV").c_str());
  }
...
{
  if(magnifyLog<=0) magnifyLog=1.2;
  ...
  //Now write away the individual dataset histos!
