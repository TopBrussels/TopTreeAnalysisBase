...
double LeptonTools::getMuonSF(double eta, double pt, string syst)
{
    ...
    for(unsigned int i=0; i<muonPt.size(); i++){
        if( (fabs(muonPt[i] - pt) <= muonPtErr[i]) || (ReReco_ && i==muonPt.size()-1 && pt > 300)){ //300 GeV is the edge of determination of the Id and Iso SFs for 2012 ReReco data (not for trigger SF, that goes up to 500) 
	    foundPt = true;
...
{
  if(muonPt.size() == 0) cout << "LeptonTools::getMuonIdoIsoSF :  No SF are loaded!!!" << endl;
  ...
  if(!foundPt) cout << "LeptonTools::getMuonIdoIsoSF :  Unknown Pt bin: " << pt << endl;
  exit(-1);
  return 1.;
}
