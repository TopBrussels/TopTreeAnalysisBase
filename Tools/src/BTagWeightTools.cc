...
float  BTagWeightTools::getSF(float pt, float eta,int flavor,string algo, int btagsyst, int mistagsyst){
  if(pt<_ptmin || pt>_ptmax)
    cout << "BTagWeightTools::getSF WARNING retrieving for pT value (" << pt << ") outside range of " << _ptmin << ","<< _ptmax << " which is fine but treat with caution..." << endl;
  if(fabs(eta)>_etamax)
...
void BTagWeightTools::setalgoWPcut(std::string algo)
{
  ...
  if(algo=="CSVL")          _algoWPcut = 0.244;
  else if(algo=="CSVM")     _algoWPcut = 0.679;
  else if(algo=="CSVT")     _algoWPcut = 0.898;
  else if(algo=="CSVV1L")   _algoWPcut = 0.405;
  else if(algo=="CSVV1M")   _algoWPcut = 0.783;
  else if(algo=="CSVV1T")   _algoWPcut = 0.920;
  else if(algo=="CSVSLV1L") _algoWPcut = 0.527;
  else if(algo=="CSVSLV1M") _algoWPcut = 0.756; 
  else if(algo=="CSVSLV1T") _algoWPcut = 0.859;
  else if(algo=="TCHPT")    _algoWPcut = 3.41;
  else if(algo=="JPL")      _algoWPcut = 0.275;
  else if(algo=="JPM")      _algoWPcut = 0.545;
  else if(algo=="JPT")      _algoWPcut = 0.790;
...
{
  //working points from https://twiki.cern.ch/twiki/bin/viewauth/CMS/BTagPerformanceOP
  if(algo_L=="CSVL")          _algoWPcut_L = 0.244;
  else if(algo_L=="CSVM")     _algoWPcut_L = 0.679;
  else if(algo_L=="CSVT")     _algoWPcut_L = 0.898;
  else if(algo_L=="CSVV1L")   _algoWPcut_L = 0.405;
  else if(algo_L=="CSVV1M")   _algoWPcut_L = 0.783;
  else if(algo_L=="CSVV1T")   _algoWPcut_L = 0.920;
  else if(algo_L=="CSVSLV1L") _algoWPcut_L = 0.527;
  else if(algo_L=="CSVSLV1M") _algoWPcut_L = 0.756; 
  else if(algo_L=="CSVSLV1T") _algoWPcut_L = 0.859;
  else if(algo_L=="TCHPT")    _algoWPcut_L = 3.41;
  else if(algo_L=="JPL")      _algoWPcut_L = 0.275;
  else if(algo_L=="JPM")      _algoWPcut_L = 0.545;
  else if(algo_L=="JPT")      _algoWPcut_L = 0.790;
  else std::cout<<" BTagWeightTools::BTagWeightTools WARNING: Working Point for algorithm not found!"<<std::endl;
	
  if(algo_T=="CSVL")          _algoWPcut_T = 0.244;
  else if(algo_T=="CSVM")     _algoWPcut_T = 0.679;
  else if(algo_T=="CSVT")     _algoWPcut_T = 0.898;
  else if(algo_T=="CSVV1L")   _algoWPcut_T = 0.405;
  else if(algo_T=="CSVV1M")   _algoWPcut_T = 0.783;
  else if(algo_T=="CSVV1T")   _algoWPcut_T = 0.920;
  else if(algo_T=="CSVSLV1L") _algoWPcut_T = 0.527;
  else if(algo_T=="CSVSLV1M") _algoWPcut_T = 0.756; 
  else if(algo_T=="CSVSLV1T") _algoWPcut_T = 0.859;
  else if(algo_T=="TCHPT")    _algoWPcut_T = 3.41;
  else if(algo_T=="JPL")      _algoWPcut_T = 0.275;
  else if(algo_T=="JPM")      _algoWPcut_T = 0.545;
  else if(algo_T=="JPT")      _algoWPcut_T = 0.790;
  else std::cout<<" BTagWeightTools::BTagWeightTools WARNING: Working Point for algorithm not found!"<<std::endl;	
}
...
float BTagWeightTools::getSFlight(float pt, float eta, string algo, int syst){
  ...
  if(syst<0)      meanminmax="min";
  else if(syst>0) meanminmax="max";
  TString tagger = algo;
...
float BTagWeightTools::getTagEff_T(float pt, float eta, int flavor)
{
  ...
  if (pt < PtMax)       xBin = inTotalNofBJets_T->GetXaxis()->FindBin(pt);		//Histograms have same binning.
  else if (pt >= PtMax) xBin = inTotalNofBJets_T->GetXaxis()->FindBin(PtMax-1);

  if (fabs(eta) < 2.4)       yBin = inTotalNofBJets_T->GetYaxis()->FindBin(fabs(eta));
  else if (fabs(eta) == 2.4) yBin = inTotalNofBJets_T->GetYaxis()->FindBin(2.4-0.01);
