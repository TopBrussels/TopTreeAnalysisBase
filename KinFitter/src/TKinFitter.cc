...
void TKinFitter::addMeasParticle( TAbsFitParticle* particle ) {
  ...
  if ( particle != 0 ) {
    ...
    cerr << "NullPointer: Measured particle points to NULL. It will not be added to the KinFitter." << endl;
  }
...
void TKinFitter::addUnmeasParticle( TAbsFitParticle* particle ) {
  ...
  if ( particle != 0 ) {
    ...
    cerr << "NullPointer: Unmeasured particle points to NULL. It will not be added to the KinFitter.";
  }
...
Int_t TKinFitter::fit() {
  ...
  do {
    ...
    if( TMath::IsNaN(currF) ) {
      cout << "KinFitter: The current value of F is NaN. Fit will be aborted." << endl;
      _status = -10;
    }
    if( TMath::IsNaN(currS) ) {
      cout << "KinFitter: The current value of S is NaN. Fit will be aborted." << endl;
      _status = -10;
    }
...
void TKinFitter::setCovMatrix( TMatrixD &V ) {
  ...
  if ( (V.GetNrows() != _nParB) || (V.GetNcols() != _nParB) ) {
    cerr << "WrongMatrixSize: Covariance matrix of measured particles needs to be a " << _nParB << "x" << _nParB << " matrix." << endl;
  } else {
    _V.ResizeTo( V );
    _V = V;
  }
...
Bool_t TKinFitter::calcV() {
  ...
  try {
    ...
    cout << "KinFitter: Failed to invert covariance matrix V. Fit will be aborted." << endl;
    _status = -10;
  }
...
Bool_t TKinFitter::calcVB() {
  ...
  try {
    ...
    cout << "KinFitter: Failed to invert matrix VB. Fit will be aborted." << endl;
    _status = -10;
  }
...
Bool_t TKinFitter::calcVA() {
  ...
  try {
    ...
    cout << "KinFitter: Failed to invert matrix VA. Fit will be aborted." << endl;
    _status = -10;
  }
...
void TKinFitter::print() {
  ...
  for (unsigned int iP = 0; iP < _measParticles.size(); iP++) {
    ...
    cout << std::setw(3) << setiosflags(std::ios::right) << iP;
    cout << std::setw(15) << setiosflags(std::ios::right) << particle->GetName();
    cout << std::setw(3) << " ";
    for (int iPar = 0; iPar < nParP; iPar++) {
      if (iPar > 0) {
	cout << setiosflags(std::ios::right) << std::setw(21) << " ";
      }
      TString colstr = "";
      colstr += parIndex;
      colstr += ":";
      cout << std::setw(4) << colstr;
      cout << std::setw(2) << " ";   
      cout << setiosflags(std::ios::left) << setiosflags(std::ios::scientific) << std::setprecision(3);
      cout << std::setw(15) << (*par)(iPar, 0);
      if(_nbIter > 0 && _status < 10) {
	cout << std::setw(15) << TMath::Sqrt( _yaVFit(iPar, iPar) );
      } else {
	cout << std::setw(15) << " ";
      }
      cout << std::setw(15) << TMath::Sqrt( (*covP)(iPar, iPar) );
      cout << "\n";
      parIndex++;
    }
  ...
  for (unsigned int iP = 0; iP < _unmeasParticles.size(); iP++) {
    ...
    const TMatrixD* par = particle->getParCurr();
    cout << std::setw(3) << setiosflags(std::ios::right) << iP;
    cout << std::setw(15) << particle->GetName();
    cout << std::setw(3) << " ";
    for (int iPar = 0; iPar < nParP; iPar++) {
      if (iPar > 0) {
	cout << setiosflags(std::ios::right) << std::setw(21) << " ";
      ...
      cout << std::setw(15) << (*par)(iPar, 0);
      if(_nbIter > 0 && _status < 10) {
	cout << std::setw(15) << TMath::Sqrt( _yaVFit(iPar+_nParB, iPar+_nParB) );
      } else {
	cout << std::setw(15) << " ";
      }
      cout << "\n";
...
void TKinFitter::printMatrix(const TMatrixD &matrix, const TString name) {
  ...
  if (!matrix.IsValid()) {
    cout << "InvalidMatrix: Matrix " << name << " is invalid." << endl;
    return;
  }
  ...
  cout << std::setfill('=') << std::setw(sw) << "=  " << name << std::setw(sw) << std::left << "  ="  << "\n"
  ...
  for (Int_t iSheet = 0; iSheet < nCols; iSheet += colsPerSheet) {
    ...
    for (Int_t iCol = iSheet; iCol < iSheet+colsPerSheet && iCol < nCols; iCol++)
      cout << std::setw(8) << iCol << "    |";
    cout << "\n" << topbar << " \n";
    for(Int_t iRow = 0; iRow < nRows; iRow++) {
      cout << std::setw(4) << iRow << " |";
      for (Int_t iCol = iSheet; iCol < iSheet+colsPerSheet && iCol < nCols; iCol++)
        cout << std::setw(12) << matrix(iRow, iCol) << " ";
      cout << "\n";
    }
  }
