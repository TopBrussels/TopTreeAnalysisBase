...
void TAbsFitParticle::calcCovMatrixDeltaY() {
  ...
  if(_covMatrixFit.GetNrows() == _nPar && _covMatrixFit.GetNcols() == _nPar)
    ...
    cerr << "WrongMatrixSize: "
      << GetName() << "::calcCovMatrixDeltaY - _covMatrixFit probably not set." << endl;
}
...
const TMatrixD* TAbsFitParticle::getPull() {
  ...
  for (int i = 0; i<_nPar; i++) {
    ...
    if (sigmaDeltaY < 0) {
      cout << "NegativeDiagonalElem: V[deltaY] has a negative diagonal element." << endl;
      _pull.Zero();
      return &_pull;
    } else {
...
void TAbsFitParticle::setParIni(const TMatrixD* parini) {
  if (parini == 0) return;
    ...
    cerr << "WrongMatrixSize: " << GetName() << "::setParIni - Matrices don't fit." << endl;
    return;
      }
}
