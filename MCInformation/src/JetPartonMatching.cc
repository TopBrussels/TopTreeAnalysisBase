...
JetPartonMatching::calculate()
{
  ...
  for(unsigned int ip=0; ip<partons.size(); ++ip){
    if( partons[ip] == TLorentzVector(0,0,0,0) ){
      emptyParton=true;
      break;
    }
  }
...
double 
JetPartonMatching::distance(const TLorentzVector& v1, const TLorentzVector& v2)
{
...
JetPartonMatching::matchingTotalMinDist()
{
  ...
  for(unsigned int ip=0; ip<partons.size(); ++ip){
    for(unsigned int ij=0; ij<jets.size(); ++ij){ 
      double dist = distance(jets[ij], partons[ip]);
					...
					std::vector<bool> & usedJets, std::vector<std::pair<double, MatchingCollection> > & distMatchVec)
{
  ...
  MatchingCollection match;
  for(unsigned int ip=0; ip<partons.size(); ++ip){
    double dist  = distance(partons[ip], jets[jetIndices[ip]]);
...
JetPartonMatching::getDistanceForParton(const unsigned int part, const unsigned int comb)
{
  ...
  return distance( jets[getMatchForParton(part,comb)], partons[part] );
}
...
JetPartonMatching::print()
{
  ...
  cout << " number of available combinations: " << getNumberOfAvailableCombinations() << "\n";
  for(unsigned int comb = 0; comb < matching.size(); ++comb) {
    cout << " -------------------------------------------- \n";
    cout << " ind. of matched jets:";
    for(unsigned int part = 0; part < partons.size(); ++part)
      cout << std::setw(4) << getMatchForParton(part, comb);
    cout << "\n";
    cout << " sumDeltaR             : " << getSumDeltaR(comb) << "\n";
    cout << " sumDeltaPt / sumDeltaE: " << getSumDeltaPt(comb) << " / "  << getSumDeltaE(comb);
    cout << "\n";
  }
  cout << "++++++++++++++++++++++++++++++++++++++++++++++";
}
